export default {
    ...
    usage: {
      ...
      turnStateBlocks: '{n} blk',
      turnStateHint: 'Ciphertext {blocks} blocks ({chars} chars); under PKCS#7 the plaintext falls in {min}–{max} bytes. Baseline is 10 blocks. Block count only brackets the plaintext to a 16-byte window, so this is a suspicion, not proof. Minted {minted}',
	  ...
	  turnStateHealthyOnly: 'New value healthy (10 blocks)',
	  turnStateSuspectOnly: 'New value suspect (not 10 blocks)',
