export default {
    accounts: {
      ...
      openai: {
        ...
        turnStateHunterDesc:
          '票到期前开窗，经勾选的代理逐个开新会话探测，摇到 292 即记入候选池。探测响应头到手即断，主要成本是每次探测的输入 token（含该模型的 base prompt）。猎到的票不再注入真实流量，手填覆写也已移除。每小时有上限；空闲门槛内没有真实请求的模型不猎。每次探测都新建一条代理连接，webshare 的 -rotate 端点因此每次换出口；其余代理按固定出口处理：探测前先解析出口 IP，同一出口只探一次，铸出 312 的出口 7 天内不再探。',
        ...
        turnStateHunterHoldDesc:
          '保留给已有配置。真实请求不再因缺票被改写或暂停；猎手只探测并记录结果。',
