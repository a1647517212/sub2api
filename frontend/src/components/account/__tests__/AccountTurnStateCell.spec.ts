...
describe('AccountTurnStateCell', () => {
  ...
  it('观测行不按票的 TTL 消失', async () => {
    vi.useFakeTimers()
    try {
      const w = render(account([], { openai_turn_state_observed: obs('m', 60) }))
  ...
  it('候选池不因 stale_after_minutes 被画成生效票', () => {
    const w = render(
  ...
  it('关掉自动接管也不再展示手填票，未降智观测仍标「最近铸出」', () => {
    const w = render(
    ...
    expect(rows(w)).toEqual(['admin.accounts.openai.turnStatePool.observedTag|other'])
  })

  it('不再把候选池票当成生效票，观测行单独展示', () => {
    const same = render(account([cand('m', 60)], { openai_turn_state_observed: obs('m', 30, 12) }))
    expect(rows(same)).toEqual(['admin.accounts.openai.turnStatePool.observedTag|m'])
    ...
    expect(rows(other)).toEqual(['admin.accounts.openai.turnStatePool.observedTag|n'])
    expect(other.findAll('.bar')[0].attributes('data-color')).toBe('purple')
  })
  ...
  it('观测行把形态数字摆出来，健康为绿', () => {
    ...
    expect(chips.map((c) => c.text())).toEqual(['332'])
    expect(chips[0].classes().join(' ')).toContain('green')
  })
  ...
  it('候选池里的 team/降智票不再上屏，健康观测仍按块数展示', () => {
    const w = render(
      account([cand('individual', 60, 10), cand('team', 60, 12), cand('degraded', 60, 11)], {
        openai_turn_state_observed: obs('team', 60, 12)
      })
    )
    expect(rows(w)).toEqual(['admin.accounts.openai.turnStatePool.observedTag|team'])
    expect(w.get('.bar').attributes('data-color')).toBe('purple')
  })
  ...
  it('猎手开着、自动接管关着：照常显示猎手状态', () => {
    ...
    const line = w.get('[data-testid="account-turn-state-hunter"]')
    expect(line.text()).toContain('hunterReady')
    expect(line.text()).not.toContain('turnStatePool.hunterNeedsAuto')
    expect(line.classes()).not.toContain('text-amber-600')
  })

  it('stale_after_minutes 是数字串时，观测行的剩余比例按它算', () => {
    const w = render(
      account([], {
        openai_turn_state_observed: obs('m', 300),
        openai_turn_state_stale_after_minutes: '10'
      })
    )
    const bar = w.get('.bar')
