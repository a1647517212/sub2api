...
func TestOpenAICodexCookieStore_ProxyChangeClearsReusedHeaders(t *testing.T) {
	store := &openAICodexCookieStore{}
	account := codexCookieTestAccount(9, AccountTypeOAuth)
	const target = "https://chatgpt.com/backend-api/codex/responses"
	h := http.Header{}
	store.Store(account, target, codexCookieUpstreamResponse())
	store.Attach(account, target, h)
	require.NotEmpty(t, h.Get("Cookie"))
	id := int64(10)
	account.ProxyID = &id
	account.Proxy = &Proxy{ID: id, Protocol: "http", Host: "127.0.0.1", Port: 8080}
	store.Attach(account, target, h)
	require.Empty(t, h.Get("Cookie"), "复用 header 也不得保留旧出口 cookie")
	store.Store(account, target, codexCookieUpstreamResponse())
	store.Attach(account, target, h)
	require.NotEmpty(t, h.Get("Cookie"))
	account.Proxy.Port = 8081
	store.Attach(account, target, h)
	require.Empty(t, h.Get("Cookie"), "同一个代理 ID 更换地址也必须隔离")
}
