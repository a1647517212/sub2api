...
func openAICodexCookieJarKey(account *Account) string {
	// Cloudflare 放行与出口有关。更换代理绑定或同一代理行的地址后不能回放旧出口的 cookie。
	proxy := "direct"
	if account.ProxyID != nil {
		proxy = strconv.FormatInt(*account.ProxyID, 10)
	}
	if account.Proxy != nil {
		digest := sha256.Sum256([]byte(account.Proxy.URL()))
		proxy += fmt.Sprintf(":%x", digest[:])
	}
	return strconv.FormatInt(account.ID, 10) + "|" + codexAccountIdentityNamespace(account) + "|" + proxy
}
...
func (s *openAICodexCookieStore) Attach(account *Account, rawURL string, headers http.Header) {
	...
	// Header 可能来自上一 attempt；空罐同样必须清掉旧账号/出口遗留值。
	headers.Del("Cookie")
